...
*   **Builder Pattern (`3_Builder.py`)**: Separates complex object construction from its representation.
*   **Adapter Pattern (`4_Adapter_pattern.py`)**: Allows incompatible interfaces to work together seamlessly.
*   **Decorator Pattern (`5_Decorator_Pattern.py`)**: Dynamically attaches new responsibilities to objects.
*   **Proxy Pattern (`6_Proxy_Pattern.py`)**: Provides a placeholder object to control access to another object.
*   **Observer Pattern (`7_Observer_pattern.py`)**: Establishes a one-to-many dependency relationship between objects.
*   **Command Pattern (`9_Command_pattern.py`)**: Encapsulates requests as objects, supporting undoable operations.
*   **MVC Pattern (`10_MVC_Pattern.py`)**: Decouples application logic, user interface, and data layers.
*   **Dependency Injection (`11_Dependency_pattern.py`)**: Promotes loose coupling by passing dependencies externally.
...
*   **Customer Sorting (`3_Sorting_customers.py`)**: Fast sorting algorithms for customer profiles.
*   **Employee Management (`4_Employee_management.py`)**: Data record structures for staff directories.
*   **Task Management (`5_Task_management.py`)**: Priority queues and task schedulers.
*   **Library System (`6_Library_management.py`)**: Book and catalog lookup systems.
*   **Financial Forecast (`7_Financial_Forecast.py`)**: Projections using data modeling.
...
*   **FastAPI**: Modern, high-performance asynchronous API endpoints, dependency injection, and automatic OpenAPI documentation.
